...
# ---------------------------------------------------------------------------
# AI Assistant support: JWT auth config
#
# The pre-existing login flow only ever returned a user object -- it never
# issued a verifiable credential. Anything that needs to know "who is making
# this request" (like the new AI assistant) needs a real, signed token
# instead of trusting a client-supplied email/customer_id. We add JWT
# issuance on top of the *existing* login/OTP flow without changing its
# response shape (the token is an additive field), so the existing frontend
# keeps working untouched.
# ---------------------------------------------------------------------------
JWT_SECRET = os.getenv("JWT_SECRET", "")
JWT_ALGORITHM = "HS256"
JWT_EXPIRES_MINUTES = int(os.getenv("JWT_EXPIRES_MINUTES", "120"))
PRODUCTS_SEED_PATH = os.path.join(os.path.dirname(__file__), "data", "products_seed.json")


def issue_customer_token(user):
    if not JWT_SECRET:
        # Fail loudly at token-issue time only (not at import time) so the
        # existing auth endpoints keep working even if an operator hasn't
        # rolled out JWT_SECRET yet -- the AI assistant simply won't be
        # usable for that customer until it is configured.
        return None
    now = datetime.utcnow()
    payload = {
        "sub": str(user["id"]),
...
def decode_customer_token(token):
    if not JWT_SECRET:
        return None
    try:
        return jwt.decode(token, JWT_SECRET, algorithms=[JWT_ALGORITHM], issuer="veeraops-backend")
...
def auth_required(view):
    """Requires a valid customer JWT. Populates g.customer with
    {id, email, username}. Used by every endpoint the AI assistant's MCP
    tools call, so a customer can never fetch another customer's data by
    guessing an id/email."""
...
def ensure_ai_order_columns(cursor):
    """Adds the extra order columns the AI assistant's tracking/status tools
    need (payment_status, tracking_number, carrier, expected_delivery,
    cancelled_at). Guarded via information_schema so it is safe to call on
    every request and never destroys existing data."""
    cursor.execute(
        """
...
def ensure_products_table(cursor):
    cursor.execute(
        ...
        CREATE TABLE IF NOT EXISTS products (
            ...
            image_query VARCHAR(255) DEFAULT NULL,
    ...
    if cursor.fetchone()["cnt"] > 0:
        return
    if not os.path.exists(PRODUCTS_SEED_PATH):
        return
    with open(PRODUCTS_SEED_PATH, "r", encoding="utf-8") as handle:
        seed_products = json.load(handle)
    for item in seed_products:
        cursor.execute(
            """
            INSERT IGNORE INTO products
                (product_id, name, description, price, category, category_title, image_query)
...
def serialize_product_row(row, base_image_url=None):
    image_url = None
    if row.get("image_query"):
        image_url = f"https://loremflickr.com/480/480/{row['image_query']}"
    return {
        "product_id": row["product_id"],
        "name": row["name"],
...
def enrich_order_row(order):
    """Derives customer-facing shipping/eligibility info from the order's
    status. There is no separate shipping-carrier integration in this
    project, so expected delivery / tracking are computed deterministically
    from data that actually exists on the order -- never invented per
    request, and stable across repeated calls for the same order."""
    ...
    if not order.get("expected_delivery") and status not in ("cancelled",):
        order["expected_delivery"] = (created_dt + timedelta(days=5)).date().isoformat()
    elif order.get("expected_delivery") and hasattr(order["expected_delivery"], "isoformat"):
        order["expected_delivery"] = order["expected_delivery"].isoformat()

    if not order.get("tracking_number") and status in ("shipped", "out_for_delivery", "delivered"):
        order["tracking_number"] = f"VEERA-TRK-{order['id']:06d}"
    if not order.get("carrier") and order.get("tracking_number"):
        order["carrier"] = "VeeraOps Logistics"

    non_cancellable_statuses = {"delivered", "cancelled", "shipped", "out_for_delivery"}
    order["cancellable"] = status not in non_cancellable_statuses
    if order.get("cancelled_at") and hasattr(order["cancelled_at"], "isoformat"):
        order["cancelled_at"] = order["cancelled_at"].isoformat()
    return order


def fetch_order_items_rows(cursor, order_id):
    cursor.execute(
        """
...
def fetch_owned_order(cursor, customer_id, order_id):
    """Central authorization choke-point: every order-detail endpoint used
    by the AI assistant goes through this, so a customer (or a confused LLM)
    can never read or act on another customer's order."""
    cursor.execute(
        """
        SELECT id, user_id, shipping_name, shipping_email, shipping_address, shipping_phone,
               total_amount, status, payment_status, tracking_number, carrier,
               expected_delivery, cancelled_at, created_at
        FROM orders
...
# ---------------------------------------------------------------------------
# Product catalog endpoints (used by the storefront and by the AI
# assistant's search_products / get_product MCP tools)
# ---------------------------------------------------------------------------
...
def list_products():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            params = []
            if query:
                clauses.append("(name LIKE %s OR description LIKE %s OR category_title LIKE %s)")
                like = f"%{query}%"
            ...
            cursor.execute(
                f"""
                SELECT product_id, name, description, price, category, category_title,
                       image_query, in_stock
                FROM products
                {where_sql}
                ORDER BY name ASC
                LIMIT %s
            ...
            rows = cursor.fetchall()
            return jsonify({"products": [serialize_product_row(r) for r in rows], "count": len(rows)}), 200
    finally:
        conn.close()
...
def get_product_detail(product_id):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            return jsonify({"product": serialize_product_row(row)}), 200
    finally:
        conn.close()


# ---------------------------------------------------------------------------
# Authenticated order endpoints for the AI assistant's MCP tools.
# Every route here requires a valid customer JWT and only ever reads/writes
# rows owned by that authenticated customer -- never a customer_id supplied
# by the caller.
# ---------------------------------------------------------------------------
...
def list_my_orders():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            ensure_ai_order_columns(cursor)
            cursor.execute(
                """
                SELECT id, user_id, shipping_name, shipping_email, shipping_address, shipping_phone,
                       total_amount, status, payment_status, tracking_number, carrier,
            ...
            for order in orders:
                order["items"] = fetch_order_items_rows(cursor, order["id"])
                order["total_amount"] = float(order["total_amount"])
                order["created_at"] = order["created_at"].isoformat() if order["created_at"] else None
                enrich_order_row(order)
            return jsonify({"orders": orders}), 200
    finally:
        conn.close()


@app.route("/api/me/orders/<int:order_id>", methods=["GET"])
@auth_required
def get_my_order(order_id):
    ...
    try:
        with conn.cursor() as cursor:
            ensure_order_tables(cursor)
            ensure_ai_order_columns(cursor)
            order = fetch_owned_order(cursor, g.customer["id"], order_id)
            if not order:
                return jsonify({"error": "Order not found"}), 404
            order["items"] = fetch_order_items_rows(cursor, order_id)
            order["total_amount"] = float(order["total_amount"])
            order["created_at"] = order["created_at"].isoformat() if order["created_at"] else None
            enrich_order_row(order)
...
@app.route("/api/me/orders/<int:order_id>/status", methods=["GET"])
@auth_required
def get_my_order_status(order_id):
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            ensure_order_tables(cursor)
...
@app.route("/api/me/orders/<int:order_id>/tracking", methods=["GET"])
@auth_required
def get_my_order_tracking(order_id):
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            ensure_order_tables(cursor)
...
@app.route("/api/me/orders/<int:order_id>/items", methods=["GET"])
@auth_required
def get_my_order_items(order_id):
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            ensure_order_tables(cursor)
            ...
            if not order:
                return jsonify({"error": "Order not found"}), 404
            return jsonify({"order_id": order_id, "items": fetch_order_items_rows(cursor, order_id)}), 200
    finally:
        conn.close()


@app.route("/api/me/orders/<int:order_id>/cancel", methods=["POST"])
@auth_required
def cancel_my_order(order_id):
    data = get_json_payload()
    confirmed = bool(data.get("confirm") is True)
        ...
        with conn.cursor() as cursor:
            ...
            if not order["cancellable"]:
                return (
                    jsonify(
                        {
                            "error": f"Order {order_id} is '{order['status']}' and is no longer eligible for cancellation.",
                            "eligible": False,
                        }
                    ),
                    409,
            ...
            if not confirmed:
                # Business rule: cancellation is a destructive action, so it
                # is a two-step flow. The caller (AI agent) must show the
                # customer this eligibility result and obtain explicit
                # confirmation before resubmitting with confirm=true.
                return (
                    jsonify(
                        {
                            "eligible": True,
                            "requires_confirmation": True,
                            "message": f"Order {order_id} is eligible for cancellation. Resubmit with confirm=true to proceed.",
                        }
                    ),
                    200,
                )
            ...
            cursor.execute(
                "UPDATE orders SET status = 'cancelled', cancelled_at = %s WHERE id = %s AND user_id = %s",
                (datetime.now(), order_id, g.customer["id"]),
            )
            conn.commit()
            return (
                jsonify(
...
@auth_required
def auth_me():
    """Used by the AI agent / MCP server to confirm a token is valid and
    resolve it to a customer, instead of trusting any customer_id supplied
    by the browser or by the LLM."""
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            user = fetch_user_by_email(cursor, g.customer["email"])
            if not user:
